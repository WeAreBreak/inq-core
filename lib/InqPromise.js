...
/*
 * promise resolved (Promises/A)
 */

function resolved(onFulfilled, onRejected) {
    var state = this._state
    ...
    if(state === 1)
        ...
        this.once('fulfilled', onFulfilled).once('error', onRejected)

    return this
}

define([ 'resolved', 'then' ], resolved)
...
define([ 'fulfilled', 'successful', 'success' ], fulfilled)
...
function execute(context) {
    ...
    if(this._state === 0) setImmediate(next)
}
