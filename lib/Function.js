...
/**
 * Wraps a task into another one that will execute the wrapt task repeadedly with given conditions and collects results into a new array.
 *
 * @param {number|function(i,res)} [times]
 *  Wrapt task will be executed 'times' times before returning the results if no error occurs.
 *  If 'times' is a function then it will be called with the count of repeats and the result of previous execution on every repeat
...
function repeat(times) {
    ...
    if(!(times instanceof Function))
        times = (function (r, i) { return this - i }).bind(+times)
    ...
    return decorateTask(function (done, recall) {
        ...
        function next(err, res) {
            if(err)
                ...
                if(Math.max(Math.round(+times(res, i++) || 0), 0))
                    fn(next, true)
                else
                    done(null, result)
            }
...
function decorateTaskWithRepeat(t) {
    return Object.defineProperty(t, 'repeat', { get: function () { return repeat.bind(t) } })
}
